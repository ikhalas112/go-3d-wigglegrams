...
export function Canvas(props: CanvasProps) {
  ...
  const touch_start = (event: React.TouchEvent<HTMLDivElement>) => {
  ...
  const touch_move = (event: React.TouchEvent<HTMLDivElement>) => {
    const touch_location = event.targetTouches[0];
      ...
      {canvas_ref.current && props.image != "" && (
        <div className="flex flex-col items-center justify-center pb-3">
          ...
          <div
            ...
            onTouchStart={(event) => touch_start(event)}
            onTouchMove={(event) => touch_move(event)}
          >
